...
<div class="card visit_div">
  ...
  <div class="card-body border-bottom py-3">
    ...
    <form method="POST" action="{{ url('dashboard/visit/store') }}" class="form-horizontal" enctype="multipart/form-data">
      @csrf
      <div class="row">
             ...
             <div class="col col-md-12">
               <select name="moken_select" class="form-control" multiple>
                 @foreach(\App\MokeemPerson::where("is_archive",0)->get() as $person)
                   <option value="{{$person->id}}" {{old('moken_select') == $person->id?"selected":""}}>{{$person->person_name}}</option>
                 @endforeach
               </select>
             </div>
           </div>
            ...
            <div class="col col-md-12">
              <select name="relative_select" class="form-control" multiple>
                @foreach(\App\MokeemRelative::all() as $relative)
                  <option value="{{$relative->id}}" {{old('relative_select') == $relative->id?"selected":""}}>{{$relative->relative_name}}</option>
                @endforeach
              </select>
            </div>
          </div>
      ...
      <div class="row form-group row_t">
        ...
        <div class="col-12 col-md-6">
          <select name="visit_type" class="form-control">
            <option value="visit_in" {{ old('visit_type') == "visit_in"?"selected":"" }}>داخلية</option>
            <option value="visit_out" {{ old('visit_type') == "visit_out"?"selected":"" }}>خارجية</option>
          </select>
      </div>
      </div>
      <div class="row form-group row_t">
        ...
        <div class="col-12 col-md-6">
          <input type="text"  name="visite_date" placeholder="" value="{{ old('visite_date') }}" class="form-control {{ $errors->has('visite_date') ? ' is-invalid' : '' }}">
      </div>
      </div>
      <div class="row form-group row_t">
        <div class="col col-md-2"><label for="text-input" class=" form-control-label">وقت الزيارة</label></div>
        <div class="col-12 col-md-6">
          <input type="text"  name="visite_time" placeholder="" value="{{ old('visite_time') }}" class="form-control {{ $errors->has('visite_time') ? ' is-invalid' : '' }}">
      </div>
      </div>
      <div class="row form-group row_t">
        <div class="col col-md-2"><label for="text-input" class=" form-control-label">مدة الزيارة</label></div>
        <div class="col-12 col-md-6">
          <input type="text"  name="visite_period" placeholder="" value="{{ old('visite_period') }}" class="form-control " disabled>
      </div>
      </div>
      <div class="row form-group row_t">
        <div class="col col-md-2"><label for="text-input" class=" form-control-label">عدد الزائرين</label></div>
        <div class="col-12 col-md-6">
          <input type="text"  name="visite_num" placeholder="" value="{{ old('visite_num') }}" class="form-control {{ $errors->has('visite_num') ? ' is-invalid' : '' }}">
      </div>
      </div>
      <div class="row form-group row_t">
        <div class="col col-md-2"><label for="text-input" class=" form-control-label">الرقم المرجعى</label></div>
        <div class="col-12 col-md-6">
          <input type="text"  name="mar_rakam" placeholder="" value="{{ old('mar_rakam') }}" class="form-control {{ $errors->has('mar_rakam') ? ' is-invalid' : '' }}">
      </div>
      </div>
